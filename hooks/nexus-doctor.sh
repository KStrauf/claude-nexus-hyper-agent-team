...
SIGNAL_BUS="$REPO_ROOT/agent-memory/signal-bus"
TRUST_LEDGER="$REPO_ROOT/agent-memory/trust-ledger"
SHADOW_MIND="$REPO_ROOT/agent-memory/shadow-mind"
TESTS_DIR="$REPO_ROOT/tests/agents"
...
if [ -f "$SETTINGS" ]; then
  if jq empty "$SETTINGS" 2>/dev/null; then
    ...
    hook_count=$(jq '[.hooks // {} | to_entries[] | .value[] | length] | add // 0' "$SETTINGS" 2>/dev/null)
    if [ "${hook_count:-0}" -gt 0 ]; then
      check_pass "Settings: valid JSON, hooks wired"
    else
...
if $SKIP_TESTS; then
  ...
  if [ -f "$TESTS_DIR/run_contract_tests.py" ]; then
    ...
    if echo "$test_output" | grep -q "FAILED\|failed\|Error"; then
      failed_count=$(echo "$test_output" | grep -c "FAIL" || true)
      check_fail "Contract tests" "$failed_count assertion(s) failed"
    else
      passed=$(echo "$test_output" | grep -oE '[0-9]+ passed' | head -1 || echo "? passed")
...
if find "$REPO_ROOT" -maxdepth 2 -name ".DS_Store" 2>/dev/null | head -1 | grep -q .; then
  ((hygiene_issues++))
